...
- **Archivos:** `backend/utils/predictor.js`, `backend/utils/thirstPredictor.js`, `backend/utils/assistant.js` (solo el system prompt).
...
- **Fix aplicado:** solo se reescribió el mensaje de `muy_atrasado`: reporta el déficit con honestidad, la recuperación la retoma la app (los checkpoints siguen) y la acción inmediata es un vaso — la unidad real que la app registra — con el único caveat científico de no intentar recuperar el total de una sola vez, sin tope para la sed: "Vas 1764ml por detrás de tu ritmo. Registra un vaso ahora y retoma tu ritmo normal — no intentes recuperar todo de una sola vez." (Descartadas en el proceso: "repartir el resto en el día", porque al no ser una unidad de la app podía incentivar subconsumo y atrasar más, y la pauta de "sorbos", que tampoco es una unidad que la app maneje.) Los mensajes de `atrasado` ("Un vaso de agua ahora te pone al día") y de `predictThirst` se conservaron como estaban — la pauta de no-atracón queda en `muy_atrasado` y en el aviso de cantidad/sobrehidratación de `logFeedback`. Al asistente (Groq) se le agregó la regla de recuperarse de a poco cuando alguien está atrasado, para que no contradiga a la app. El detalle "el agua se absorbe mejor si no se toma en grandes cantidades" queda en la advertencia de no recuperar todo de una sola vez.
- **Verificado:** escenarios forzados en local — muy atrasado (defícit 1764ml → "Registra un vaso ahora y retoma tu ritmo normal — no intentes recuperar todo de una sola vez."), atrasado (déficit 684ml → "Un vaso de agua ahora te pone al día.", sin cambios) y sed probable (mensaje original, sin cambios); backend carga sin errores.
- **Commit:** `0615039` — 2026-08-13 (branch `fix/ponerse-al-dia-sin-atracón`, mergeada a main).
