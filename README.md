...
## 10. Arreglos tras uso real (segunda vuelta)

- **La app asumía que siempre te despiertas a la hora fija del perfil**: si
  configuraste 7am pero te levantas a las 11am, los checkpoints ya
  "esperaban" ~4 horas de consumo antes de que abrieras la app, empujando
  a tomar mucha agua de golpe para "ponerte al día" — justo lo contrario
  de lo que la app debería promover. Ahora la hora de despertar se
  recalibra sola: si pasan más de 90 minutos de la hora configurada sin
  ningún registro (o el primer trago del día llega mucho más tarde de lo
  esperado), la hora efectiva se ajusta a ~30 min antes de ese momento en
  vez de acumular una deuda irreal. Probé 4 escenarios (despertar normal,
  despertar tarde sin registrar, primer trago cerca de la hora normal,
  primer trago muy tarde) y los cuatro se comportan como corresponde. Ver
  `backend/utils/dailyStatus.js` (`effectiveWakeTime`).
- **Notificaciones push en celular sin explicación clara**: las
  restricciones son reales del sistema operativo, no de la app — HTTPS
  real es obligatorio (no funciona probando por IP local), y en
  iPhone/iPad **Safari solo permite push si instalaste la app** ("Agregar
  a pantalla de inicio") y la abres desde ese ícono, nunca desde una
  pestaña normal, sin importar qué permisos hayas dado en Ajustes. Ahora
  Perfil muestra el motivo exacto de entrada en vez de un botón que solo
  falla en silencio al tocarlo. Ver `frontend/src/push.js`
  (`getPushUnavailableReason`).
- **Historial mostraba el día de hoy hasta abajo**: la lista se ordenaba
  igual que la respuesta del backend (más antiguo primero). Ahora el
